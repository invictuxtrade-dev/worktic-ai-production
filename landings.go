...
var landingTpl = template.Must(template.New("landing-premium").Funcs(template.FuncMap{
	...
	"channelIcon": landingChannelIcon,
}).Parse(`<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8">
...
.channel-icon svg,.floating-channel svg,.mobile-channel svg{width:58%;height:58%;display:block;fill:currentColor}.channel-icon{font-size:0;color:#fff;box-shadow:0 8px 20px rgba(20,14,35,.12)}.channel-whatsapp{background:#25d366!important;color:#fff!important}.channel-telegram{background:#229ed9!important;color:#fff!important}.channel-messenger{background:linear-gradient(135deg,#00b2ff,#7b2cff 55%,#ff2d8d)!important;color:#fff!important}.channel-instagram{background:radial-gradient(circle at 30% 105%,#ffd600 0 20%,#ff7a00 32%,#ff0169 55%,#d300c5 75%,#7638fa 100%)!important;color:#fff!important}.channel-facebook{background:#1877f2!important;color:#fff!important}.channel-linkedin{background:#0a66c2!important;color:#fff!important}.channel-tiktok{background:#111!important;color:#fff!important}.channel-youtube{background:#ff0000!important;color:#fff!important}.channel-link{background:linear-gradient(135deg,var(--a),var(--a2))!important;color:#fff!important}.floating-channel{transition:transform .18s ease,box-shadow .18s ease}.floating-channel:hover{transform:translateY(-2px) scale(1.04);box-shadow:0 18px 38px rgba(25,16,43,.24)}.channel-pill .channel-icon{flex:0 0 auto}.contact-card .channel-icon svg{width:55%;height:55%}
</style>
</head>
...
{{if and .Premium.ShowFloatingChannels .Channels}}<div class="floating-channels">{{range .Channels}}<a class="floating-channel channel-{{.Type}}" href="{{.URL}}" title="{{.Label}}" aria-label="Abrir {{.Label}}" target="_blank" rel="noopener">{{channelIcon .Type}}</a>{{end}}</div>{{end}}
{{if .Premium.ShowStickyCTA}}<div class="mobile-sticky">{{range .Channels}}{{if eq .Type "whatsapp"}}<a class="mobile-channel channel-whatsapp" href="{{.URL}}" aria-label="Abrir WhatsApp" target="_blank" rel="noopener">{{channelIcon .Type}}</a>{{end}}{{end}}<a class="btn" href="{{.PrimaryURL}}">{{.PrimaryCTA}}</a></div>{{end}}
</body></html>`))
