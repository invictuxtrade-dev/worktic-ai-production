...
(() => {
  ...
  async function loadPromos(){
    const el=q('#wtAdminPromosTable'); if(!el)return; el.innerHTML='<div class="wt-admin-empty">Cargando códigos…</div>';
    try{const items=await api('/api/admin/promo-codes');el.innerHTML=items.length?`<div class="wt-admin-table-wrap"><table class="wt-admin-table"><thead><tr><th>Código</th><th>Descuento</th><th>Inicio</th><th>Vencimiento</th><th>Usos</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${items.map(x=>`<tr><td><b>${safe(x.code)}</b></td><td><b>${x.discount_percent}%</b></td><td>${fmtDate(x.starts_at)}</td><td>${fmtDate(x.expires_at)}</td><td>${x.uses||0}</td><td><span class="wt-admin-badge ${x.active?'active':'blocked'}">${x.active?'Activo':'Inactivo'}</span></td><td><div class="wt-admin-actions"><button class="secondary" data-promo-edit="${x.id}">Editar</button><button class="wt-danger" data-promo-delete="${x.id}">Eliminar</button></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="wt-admin-empty"><b>No hay códigos promocionales</b><span>Crea el primero para ofrecer descuentos temporales.</span></div>';qa('[data-promo-edit]',el).forEach(b=>b.onclick=()=>promoForm(items.find(x=>x.id===Number(b.dataset.promoEdit))));qa('[data-promo-delete]',el).forEach(b=>b.onclick=()=>promoDelete(Number(b.dataset.promoDelete)));}catch(e){el.innerHTML=`<div class="wt-admin-empty"><b>Error al cargar</b><span>${safe(errMsg(e))}</span></div>`}}
  function promoForm(x){x=x||{active:true,discount_percent:10};const local=v=>v?new Date(v).toISOString().slice(0,16):'';modal(`<div class="modal-heading"><span class="eyebrow">PROMOCIONES</span><h2>${x.id?'Editar':'Nuevo'} código promocional</h2><p>El descuento se calcula y valida siempre en el servidor.</p></div><div class="form"><div class="form-section"><div class="grid2"><label>Código<input id="wtPromoCode" value="${safe(x.code||'')}" placeholder="WORKTIC20"></label><label>Descuento porcentual<input id="wtPromoDiscount" type="number" min="0.01" max="100" step="0.01" value="${x.discount_percent||10}"></label><label>Inicio de vigencia<input id="wtPromoStart" type="datetime-local" value="${local(x.starts_at)}"></label><label>Fecha de vencimiento<input id="wtPromoEnd" type="datetime-local" value="${local(x.expires_at)}"></label></div><label class="check"><input id="wtPromoActive" type="checkbox" ${x.active!==false?'checked':''}> Código activo</label></div><div class="actions"><button type="button" id="wtPromoSave">Guardar código</button><button value="cancel" class="secondary">Cancelar</button></div></div>`);q('#wtPromoSave').onclick=async()=>{const data={...x,code:q('#wtPromoCode').value.trim().toUpperCase(),discount_percent:Number(q('#wtPromoDiscount').value),starts_at:q('#wtPromoStart').value?new Date(q('#wtPromoStart').value).toISOString():'',expires_at:q('#wtPromoEnd').value?new Date(q('#wtPromoEnd').value).toISOString():'',active:q('#wtPromoActive').checked};if(!data.code||!data.expires_at)return formError('Código y vencimiento son obligatorios.');try{await api('/api/admin/promo-codes',{method:x.id?'PUT':'POST',body:JSON.stringify(data)});q('#modal').close();notify('Código promocional guardado');loadPromos()}catch(e){formError(errMsg(e))}}}
  async function promoDelete(id){if(!await ask('Eliminar código','El código dejará de funcionar inmediatamente.','Eliminar',true))return;try{await api('/api/admin/promo-codes?id='+id,{method:'DELETE'});notify('Código eliminado');loadPromos()}catch(e){notify(errMsg(e))}}

  const oldActivateTab=activateTab; activateTab=function(name){oldActivateTab(name);if(name==='promos')loadPromos()};
  const promoBtn=q('#wtAdminNewPromo');if(promoBtn)promoBtn.onclick=()=>promoForm();
})();
