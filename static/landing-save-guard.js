(() => {
  ...
  const collectLanding = item => ({
    ...
    secondary_cta: ($('#lpSecondaryCTA')?.value || item.secondary_cta || '').trim(),
    secondary_url: ($('#lpSecondaryURL')?.value || item.secondary_url || '').trim(),
    ...
    premium_json: typeof window.collectLandingPremiumV2 === 'function' ? window.collectLandingPremiumV2() : (item.premium_json || '{}'),
